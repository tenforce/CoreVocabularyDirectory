...
- 2 :: wait for the system to completely restart (and mount of the
       ...
       not correct).
- 3 ::  dos2unix is also installed in the case that there are line
        endings which are incorrect. Once in /vagrant at the command
        line the previously described commands should allow building
