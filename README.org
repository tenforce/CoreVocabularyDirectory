...
further information on the virtuoso, the documentation of virtuoso
should be consulted. The virtuoso service will have the following
configuration, changes to the production system structure should be
duplicated here for development testing. The following are the current
setup instructions, which are used by the current dockerfile.
...
* Monitoring the deployed service on the production system 
