...
RUN cd /var/lib/virtuoso-opensource-6.1/db \
    ...
    && ( cat /var/lib/virtuoso-opensource-6.1/vsp/mdr/vhost_export.vspx.isql ; echo "ld_dir('/var/lib/virtuoso-opensource-6.1/db/data', '*.ttl', 'http://vocabs.tenforce.com/DAV');\nrdf_loader_run();\nexec('checkpoint');\nshutdown();" ) | isql-vt -U dba -P dba
